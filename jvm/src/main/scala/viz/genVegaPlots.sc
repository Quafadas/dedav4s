...
val caseStub = """case class BarChart(override val mods : JsonMod=List())(using PlotTarget) extends FromUrl(SpecUrl.BarChart)"""

val classDefs = for( plot <-codeGenFromList) yield { 
    s"""case class ${plot._2}(override val mods : JsonMod=List())(using PlotTarget) extends FromUrl(SpecUrl.${plot._2})"""         
}
