...
val codeGen = for(p <- previews.asScala) yield{
    ...
    val className =         p.text.replace(" ", "").replace("(", "_").replace(")", "").replace(".", "_").replace("/", "_")
        .replace("-", "_").replace("+", "_").replace("1", "One").replace("2", "Two").replace("“", "").replace("”", "")
        .replace("’", "").replace(",", "").replace("&", "") //classname
    
    val finalCN = if (found.contains(className)) {        
...
val specUrlStub = for( plot <-codeGenFromList) yield { 
    s"""case ${plot._2}Lite extends SpecUrl("$exampleStub${plot._1}", VegaLite) """         
}

val toPasteSpecUrls = "\n    " + specUrlStub.mkString("\n    ")
show(toPasteSpecUrls)

val classDefs = for( plot <-codeGenFromList) yield { 
    s"""case class ${plot._2}Lite (override val mods : JsonMod=List())(using PlotTarget) extends FromUrl(SpecUrl.${plot._2}Lite)"""         
}
